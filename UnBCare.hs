...
buscaEstoque :: Medicamento -> EstoqueMedicamentos -> Bool
buscaEstoque _ [] = False
buscaEstoque m (h:t)
   | m == mh = True
   | otherwise = buscaEstoque m t
   where (mh, _) = h

buscaEstoqueCASE0 :: Medicamento -> EstoqueMedicamentos -> Bool
buscaEstoqueCASE0 _ [] = False
buscaEstoqueCASE0 m (h:t)
   | m == mh && q > 0 = True
   | otherwise = buscaEstoqueCASE0 m t
   where (mh, q) = h
...
comprarMedicamento m q e
   | buscaEstoque m e == True = comprarMedicamentoAUX m q e
   | otherwise = (m,q):e
comprarMedicamentoAUX m q (h:t)
   | m == mh = (mh,qh+q):t
   | otherwise = h : comprarMedicamentoAUX m q t
   where (mh,qh) = h
...
tomarMedicamento m e
   | buscaEstoqueCASE0 m e == True = Just (tomarMedicamentoAUX m e) 
   | otherwise = Nothing
tomarMedicamentoAUX m (h:t)
   | m == mh = (mh,qh-1):t
   | otherwise = h : tomarMedicamentoAUX m t
   where (mh,qh) = h
...
consultarMedicamento m (h:t)
   | m == mh = q
   | otherwise = consultarMedicamento m t
   where (mh, q) = h
